...
parser.add_argument(
    "LIMIT", nargs="?", default=None, type=int, help="Maximum number of posts to show"
)
...
if len(posts) == 0:
    ...
    for post in posts:
        print(
            f'<li><a href="{post.href}">{post.title}</a><span class="date">{post.date.strftime(DATE_FORMAT)}</span>',
            end="",
        )
